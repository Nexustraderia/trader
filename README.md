...
Para candles intraday, o bot usa exclusivamente a IQ Option em modo somente leitura. Se a conexão, o catálogo ou os candles da IQ Option falharem, nenhuma análise, sinal ou liquidação é executada. As credenciais devem ser configuradas somente no Render, nunca no GitHub.

Quando `AUTO_INCLUDE_IQ_ASSETS=true`, um processo em segundo plano atualiza a cada cinco minutos o catálogo de ativos abertos retornado pela IQ Option (`binary`, `turbo` e `digital`). Somente ativos com código de candles conhecido pela biblioteca são adicionados ao scanner. Se a sessão IQ Option estiver indisponível, o scanner fica em `AGUARDAR` e não gera sinais. OTC só é incluído quando aparece aberto no catálogo da própria IQ Option.
